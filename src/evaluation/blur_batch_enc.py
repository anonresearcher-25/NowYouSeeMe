...
# Cloud endpoint (mock rn)
CLOUD_ENDPOINT = "https://example.com/upload"
    ...
    try:
                        ...
                        if len(landmarks):
                            ...
                            if frame_count % ENCRYPTION_FRAME_INTERVAL == 0:
                                ...
                                try:
                                    ...
                                    total_encryption_times.append((end_encryption_time - start_encryption_time) * 1000)
                                    encrypted_payload.append(encrypted)
                                    if encrypted_payload:
                                        try:
                                            # Uncomment to enable real transmission
                                            # response = requests.post(CLOUD_ENDPOINT, json={"payload": encrypted_payload})
                                            # response.raise_for_status()
                                            # print(f"Encrypted and sent frame {frame_count} face+landmark data to cloud")
                                            None
                                        except Exception as e:
                                            print(f"Failed to send encrypted data: {e}")
                                except Exception as enc_err:
                                    print(f"Encryption error on frame {frame_count}: {enc_err}")
        ...
        if total_inference_times:
            ...
            if total_encryption_times:
                avg_enc = (sum(total_encryption_times)/len(total_encryption_times))
            ...
            summary_stats = {
                ...
                "avg_encryption_ms:": round(avg_enc, 4),
