...
class SCRFD:
    ...
    def forward(self, img, threshold):
        ...
        # list1.append((end-start).total_seconds()*1000)
...
if __name__ == '__main__':
    detector = SCRFD(model_file='det_500m.onnx')
    detector.prepare(-1)

    cap = cv2.VideoCapture("../input/test_input_1.mp4")
    ...
    while cap.isOpened():
        ...
        # bboxes, kpss = detector.detect(rgb_frame, 0.5)
        tb = datetime.datetime.now()
        inference_times.append((tb-ta).total_seconds()*1000)
        ...
        for i in range(bboxes.shape[0]):
            ...
            if kpss is not None:
                ...
                for kp in kps:
                    ...
                    cv2.circle(rgb_frame, tuple(kp) , 1, (0,0,255) , 2)
    ...
    print(f"Average Inference Time: {avg_inference:.2f}")
    print(f"Theoretical Max FPS: {1000/avg_inference:.2f}")
