...
def read_thread(config, cap):
    ...
    while cap.isOpened():
        ...
        read_count += 1
        if not ret or read_count > config.CAM_FRAMES:
            read_queue.put(None)  # Sentinel to signal end of stream
            break
...
def process_thread(config, detector, interpretor, input_details, output_details, model, public_key):
    ...
    while True:
        ...
        # rgb_frame = cv2.cvtColor(output_frame, cv2.COLOR_RGB2BGR)
...
if __name__ == "__main__":
    ...
    # All 3 Ml models be pre-loaded in the glasses at boot time
        ...
        if config.SAVE_OUTPUT:
            # Read the first frame to get its dimensions (height, width)
            ret, first_frame = cap.read()
            if ret:
                h, w = first_frame.shape[:2]
                out = cv2.VideoWriter(config.OUTPUT_VIDEO_PATH, cv2.VideoWriter_fourcc(*"mp4v"), 30, (w, h))
                # IMPORTANT: Put the first frame into the queue so it gets processed!
                read_queue.put(first_frame)
            else:
